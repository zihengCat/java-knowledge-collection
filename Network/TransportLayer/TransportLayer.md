...
![Network-TransportLayer-TCPThreeWayHandshake][Network-TransportLayer-TCPThreeWayHandshake]

> 图：TCP 三次握手示意图

建立 TCP 连接需要三次握手，假设 A 为客户端，B 为服务器端。

- 连接建立前，A 处于 CLOSED 关闭状态，B 处于 LISTEN 监听状态，等待客户的连接请求。

- A 向 B 发送连接请求 TCP 报文，SYN=1，ACK=0，选择一个初始序号`x`。

- B 收到连接请求报文，如果同意建立连接，则向 A 发送连接确认报文，SYN=1，ACK=1，确认号为`x+1`，同时也选择一个初始序号`y`。

- A 收到 B 的连接确认报文后，还要向 B 发出确认，ACK=1，确认号为`y+1`，序号为`x+1`。

- B 收到 A 的确认后，连接建立。

三次握手为了确认客户端与服务端的网络连接通畅。
...
![Network-TransportLayer-TCPFourWayHandshake][Network-TransportLayer-TCPFourWayHandshake]

> 图：TCP 四次挥手示意图

释放 TCP 连接需要四次挥手。假设 A 为客户端，B 为服务器端。以下描述不讨论序号和确认号，因为序号和确认号的规则比较简单。并且不讨论 ACK，因为 ACK 在连接建立之后都为 1。

- 需要释放连接，A 发送连接释放报文，FIN=1，自身进入 FIN_WAIT_1 状态。

- B 收到之后发出确认报文，自身进入 CLOSE_WAIT 状态，此时 TCP 属于半关闭状态，B 能向 A 发送数据但是 A 不能向 B 发送数据。

- A 收到确认报文，自身进入 FIN_WAIT_2 状态。

- 当 B 不再需要连接时，发送连接释放报文，FIN=1，进入 LAST_ACK 状态。

- A 收到后发出确认报文，进入 TIME_WAIT 状态，等待 2 MSL（最大报文存活时间）后释放连接。

- B 收到 A 的确认后释放连接。

四次挥手的原因

客户端发送了 FIN 连接释放报文之后，服务器收到了这个报文，就进入了 CLOSE_WAIT 状态。这个状态是为了让服务器端发送还未传送完毕的数据，传送完毕之后，服务器会发送 FIN 连接释放报文。
...
![Network-TransportLayer-TCPSlidingWindow][Network-TransportLayer-TCPSlidingWindow]
...
[Network-TransportLayer-TCPThreeWayHandshake]: ../../images/Network-TransportLayer-TCPThreeWayHandshake.png
