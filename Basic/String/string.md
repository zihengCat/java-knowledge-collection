...
在 Java 中，字符串也是一个类，字符串的分配和其他对象分配一样，需要消耗高昂的时间和空间，而且字符串的使用非常频繁。JVM 为了提高性能和减少内存开销，在实例化字符串时会进行一些优化：**使用字符串常量池**。
...
1. 操作符`==`，如果作用于基本数据类型（`byte`、`short`、`char`、`int`、`long``、`float`、`double`、`boolean`），则直接比较其存储的"值"是否相等；如果作用于引用类型（`Object`），则比较它们所指的对象地址是否相等。

2. `equals()`方法是`Object`基类中定义的方法，所有类都继承该方法（所有类都是`Object`类的子类）。在`Object`基类中，`equals()`方法比较两个对象的地址是否相等（等同于`==`操作符）

3. 子类可以重写`equals()`方法，实现出自己的比较方式（如：`String`类）。
