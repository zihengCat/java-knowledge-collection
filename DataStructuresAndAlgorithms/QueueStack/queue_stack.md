...
队列先进先出，栈后进先出，使用队列实现栈结构，需要使用到两个队列：将元素在主副队列中来回搬运，实现出栈结构后进先出的性质。

- 压栈操作：元素正常入主队列。

- 弹栈操作：先将主队列中的元素存入副队列，只留下最后一枚队尾元素（即：栈顶元素），将该元素返回，最后把副队列中的元素放回主队列。

# 代码实现

为了简单，直接使用标准库中的`java.util.Queue`作为队列，并复用先前写好的测试代码、`Stack`接口、栈异常类。
